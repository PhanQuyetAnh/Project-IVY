...
<main id="main" style="padding: 20px; font-family: Arial, sans-serif; max-width: 1100px; margin: 100px 30px 20px 350px; background-color: #ffffff; border-radius: 20px;">
    ...
    <c:forEach var="user" items="${inactiveUsers}">
        <div class="user-info" style="border: 1px solid #ccc; border-radius: 10px; padding: 15px; margin-bottom: 15px; color: #2c3e50; text-align: center;
            ...
            <p style="margin: 5px 0; font-size: 16px;"><strong style="color: #2980b9;">Ngày sửa đổi:</strong> ${user.modifiedDate}</p>
            <button style="margin: 5px 0; padding: 10px 20px; background: #123271; border-radius: 10px; border: none; outline: none; font-size: 16px; font-weight: bold; color: ${user.active ? '#27ae60' : '#e74c3c'};">
                <c:choose>
                    <c:when test="${user.active}">Hoạt động</c:when>
